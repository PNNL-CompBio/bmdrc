...
class ProportionalClass():
    ...
    @abstractmethod
    def fit_models(self, gof_threshold = 0.1, aic_threshold = 2, model_selection = "lowest BMDL", diagnostic_mode = False, parallelize = True, n_cores = None):
        fit_the_models(self, gof_threshold, aic_threshold, model_selection, diagnostic_mode, parallelize, n_cores)
    ...
    def response(self, responsename):
        ...
        if min(self._df[responsename]) < 0 or max(self._df[responsename]) > 1:
            raise Exception("The response column must range in values from 0 to 1. Filter out NAs before using the ProportionalClass.")
